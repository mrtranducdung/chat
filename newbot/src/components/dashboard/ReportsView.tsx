...
import { Search, Bell, Bot, Filter, ChevronRight, AlertCircle, CheckCircle2, TrendingUp, TrendingDown, DollarSign, Activity, Users, Briefcase, Zap, FileText, MessageSquare, Plus, Mail, Building, PieChart, Download, Send, Loader2 } from 'lucide-react';
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between border-b pb-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-xl font-semibold text-slate-800">Reports (Auto reporting + Distribution)</h2>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <h3 className="font-semibold text-slate-800">Report Library</h3>
            ...
            {[
              ...
              <div key={idx} className="bg-white p-4 rounded-xl shadow-sm border border-slate-200 hover:border-indigo-300 transition-colors cursor-pointer">
                ...
                <div className="space-y-2 text-xs text-slate-600">
                  <p><span className="font-medium text-slate-500">Last Generated:</span> {report.last}</p>
                  <p><span className="font-medium text-slate-500">Recipients:</span> {report.recipients}</p>
                  <p><span className="font-medium text-slate-500">Sources:</span> {report.sources}</p>
                </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-semibold text-slate-800 flex items-center gap-2">
              <Bot className="text-indigo-600" /> Report Builder
            </h3>
          </div>
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Step 1: Choose Template</label>
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Step 2: Choose Scope</label>
              <div className="flex gap-2">
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-2">Step 3: Sections</label>
            ...
            <div className="pt-4 border-t border-slate-100 flex gap-2 flex-col">
              {isSuccess && (
                <div className="bg-emerald-50 text-emerald-700 p-2 rounded-lg text-sm flex items-center gap-2 mb-2">
                  <CheckCircle2 size={16} /> Report generated and sent!
                </div>
              )}
              <div className="flex gap-2">
                <button 
                ...
                >
                  {isGenerating ? <><Loader2 size={16} className="animate-spin" /> Processing...</> : 'Approve & Send'}
                </button>
              </div>
            </div>
          </div>
